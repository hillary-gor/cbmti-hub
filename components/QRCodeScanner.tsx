...
export const QRCodeScanner: React.FC<QRCodeScannerProps> = ({ onScanCompleteAction, onScanError }) => { // Use new prop name
  ...
  }, [onScanCompleteAction]); // Update dependency
  ...
  const handleError = useCallback((errorMessage: string) => {
    if (!errorMessage.includes('No QR code found')) {
      ...
      setScanMessage(`Scan error: ${errorMessage}. Please ensure good lighting.`);
    ...
    return () => {
      if (scannerRef.current && isScanning) {
        scannerRef.current.clear().then(() => {
          ...
          console.warn("Failed to clear html5QrcodeScanner. Maybe it's already stopped or an error occurred?", err);
        });
      }
    };
  ...
  return (
    <div className="flex flex-col items-center p-4 bg-white rounded-lg shadow-md">
      <div id={qrCodeId} className="w-full max-w-sm h-64 border border-gray-300 rounded-md overflow-hidden"></div>
