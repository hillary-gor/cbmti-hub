...
export default function LecturerQRPage() {
  ...
  const [sessionDetails, setSessionDetails] = useState<SessionWithCourseAndIntake | null>(null);
  ...
  const fetchSessionAndAttendance = useCallback(async () => {
    ...
    if (sessionError || !session) {
      console.error('Session fetch failed. Details:',
    ...
    if (courseIdForStudents) {
      ...
      if (!countError && count !== null) {
        ...
        console.warn('Could not fetch total student count:', countError?.message || 'Unknown error');
        setTotalStudents(0);
      }
    } else {
      console.warn('Could not determine course_id for total student count (missing course_intakes data).');
      setTotalStudents(0);
    }
    ...
    if (!attendanceCountError && currentCount !== null) {
      ...
      console.warn('Could not fetch current attendance count:', attendanceCountError?.message || 'Unknown error');
    ...
    if (session.is_attendance_active) {
      ...
      const lastGenerated = new Date(session.last_qr_code_generated_at || new Date());
      const timeElapsed = (new Date().getTime() - lastGenerated.getTime()) / 1000;
      setQrCountdown(Math.max(0, QR_REFRESH_INTERVAL_MS / 1000 - Math.floor(timeElapsed)));
    } else {
  ...
  const handleGenerateNewQrCode = useCallback(async () => {
    ...
    if (result.success) {
      ...
      toast.error(`Failed to update QR: ${result.error}`, { id: 'qr-update-toast' });
  ...
  const handleStartAttendance = async () => {
    ...
    if (result.success) {
      ...
      toast.error(`Error starting attendance: ${result.error}`, { id: 'start-attendance-toast' });
  ...
  const handleEndAttendance = async () => {
    ...
    if (result.success) {
      ...
      toast.error(`Error ending attendance: ${result.error}`, { id: 'end-attendance-toast' });
      ...
      .on(
        ...
        async (payload) => {
          ...
          if (!error && count !== null) {
            ...
            console.warn('Realtime fetch failed to update attendance count:', error?.message || 'Unknown error');
          }
        }
      )
      .subscribe();
  ...
  useEffect(() => {
    if (isAttendanceActive && !currentQrValue) {
      console.log('Attendance active and no QR value. Triggering initial QR generation.');
  ...
  if (isLoading || !sessionDetails) {
    return <div className="text-center text-gray-600 p-8">Loading session details...</div>;
  }
  ...
  return (
    <div className="container mx-auto p-4 md:p-8">
      ...
      <p className="text-gray-600 mb-4">
        Intake: {intake_label} | Date: {new Date(sessionDetails.session_date).toLocaleDateString('en-KE')} | Time: {sessionDetails.start_time.substring(0, 5)} - {sessionDetails.end_time.substring(0, 5)}
      </p>

      {!isAttendanceActive ? (
        <Button onClick={handleStartAttendance} className="mb-6 bg-green-600 hover:bg-green-700 text-white shadow-md transition-colors duration-200">
        ...
        <>
          <div className="mb-6 p-4 border rounded-lg bg-yellow-50 flex flex-col md:flex-row items-center justify-between shadow-sm">
            <p className="font-medium text-yellow-800 text-center md:text-left mb-2 md:mb-0">
              Attendance is ACTIVE. QR code refreshes in: <span className="font-bold" aria-live="polite">{qrCountdown}</span> seconds
            </p>
            <Button onClick={handleEndAttendance} className="bg-red-600 hover:bg-red-700 text-white shadow-md transition-colors duration-200 w-full md:w-auto">
              End Attendance
            </Button>
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-8 items-center justify-center">
            <div className="flex-1 text-center bg-white p-6 rounded-lg shadow-md border border-gray-100">
              <h3 className="text-xl font-medium mb-4 text-gray-800">Scan this QR Code</h3>
              {currentQrValue ? (
            ...
            <div className="flex-1 text-center bg-gray-50 p-6 rounded-lg shadow-inner border border-gray-200">
              <h3 className="text-xl font-medium mb-4 text-gray-800">Current Attendance</h3>
