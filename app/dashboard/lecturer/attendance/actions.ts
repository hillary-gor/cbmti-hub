...
export async function createSession(
  ...
  if (profileError || !userProfile || userProfile.role !== "lecturer") {
    console.warn(
      "Unauthorized attempt to create session by user:",
      user.id,
      "Role:",
      userProfile?.role,
    );
    return {
      success: false,
      error: "Access denied. Only lecturer can create sessions.",
    };
  }
  ...
  try {
    ...
    if (!courseId || !intakeId || !sessionDate || !startTime || !endTime) {
      console.error(
        "Validation error in createSession: Missing required fields.",
        { courseId, intakeId, sessionDate, startTime, endTime },
      );
      return {
        success: false,
        error: "All session fields including course and intake are required.",
      };
    }

    console.log(
      "Attempting to create session for Course ID:",
      courseId,
      "Intake ID:",
      intakeId,
      "Lecturer ID:",
    ...
    if (ciFetchError) {
      ...
      if (
        ciFetchError.code === "PGRST116" ||
        ciFetchError.message === "No rows found"
      ) {
        console.log(
          "No existing course_intake found for this pair. Attempting to create a new one...",
        );
        ...
        if (newCiInsertError) {
          console.error(
            "ERROR: Failed to create new course_intake entry:",
            newCiInsertError,
          );
          return {
            success: false,
            error: `Failed to prepare session (could not create course-intake link): ${newCiInsertError.message}`,
          };
        }
        courseIntakeId = newCi.id;
        console.log(
          "Successfully created new course_intake with ID:",
          courseIntakeId,
        );
      } else {
        // Handle other unexpected database errors during course_intake fetch
        console.error(
          "ERROR: Unhandled database error fetching course_intake:",
          ciFetchError,
        );
        return {
          success: false,
          error: `Failed to prepare session (DB error): ${ciFetchError.message}`,
        };
      }
    ...
    if (!courseIntakeId) {
      console.error(
        "ERROR: courseIntakeId is NULL or undefined after processing course_intakes logic. This should not happen.",
      );
      return {
        success: false,
        error:
          "Internal error: Could not establish a valid course-intake link.",
      };
    }

    // --- Step 2: Insert the new session into the 'sessions' table using course_intake_id ---
    console.log(
      "Attempting to insert session into public.sessions with course_intake_id:",
      courseIntakeId,
    );
...
export async function generateQrCode(
  ...
  if (profileError || userProfile?.role !== "lecturer") {
    return {
      success: false,
      error: "Access denied. Only lecturer can manage attendance.",
    };
  }
  ...
  const { data: session, error: sessionError } = await supabase
    .from("sessions")
    .select(
      "is_attendance_active, last_qr_code_generated_at, last_qr_code_value, lecturer_id",
    )
    .eq("id", sessionId)
    .single();
  ...
  if (!session.is_attendance_active) {
    return {
      success: false,
      error: "Attendance is not active for this session.",
    };
  }
  ...
  const timeDiffSeconds =
    (currentTime.getTime() - lastGeneratedTime.getTime()) / 1000;
  ...
  if (timeDiffSeconds >= 30 || !session.last_qr_code_generated_at) {
    ...
    if (updateError) {
      console.error(
        "Error updating session QR in generateQrCode:",
        updateError,
      );
    ...
    return {
      success: true,
      message: "QR not yet due for refresh.",
      newQrValue: session.last_qr_code_value || undefined,
    };
  }
}
