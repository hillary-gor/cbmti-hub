...
const StepContacts = () => {
  ...
  const nextOfKinErrors = errors.next_of_kin as FieldErrors<
    FormData["next_of_kin"]
  >;
  const emergencyErrors = errors.emergency_contact as FieldErrors<
  ...
  const getEmergencyError = <T extends keyof FormData["emergency_contact"]>(
    field: T,
  ) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <fieldset>
        ...
        <div>
          <input
            {...register("emergency_contact.full_name")}
            className="input"
            placeholder="Full Name"
          />
          <p className="text-red-500 text-sm">
            {getEmergencyError("full_name")}
          </p>
        </div>
        <div>
          <input
          ...
          />
          <p className="text-red-500 text-sm">
            {getEmergencyError("phone_number")}
          </p>
        </div>
        <div>
          <input
