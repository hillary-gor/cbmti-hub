...
export default function StudentAttendancePage() {
  ...
  const [attendanceStatus, setAttendanceStatus] = useState<
    "pending" | "marked" | "error" | null
  >(null);
  const [errorMessage, setErrorMessage] = useState<string | null>(null);

  // State for user-specific data
  const [enrolledCourses, setEnrolledCourses] = useState<
    EnrolledCourseDisplay[] | null
  >(null);
  const [userName, setUserName] = useState<string | null>(null);
  const [lastMarkedAttendance, setLastMarkedAttendance] = useState<{
    courseTitle: string;
    markedAt: string;
  } | null>(null);
  ...
  const fetchAllAndCategorizeSessions = useCallback(async () => {
    ...
    if (userDetailsError && userDetailsError.code !== "PGRST116") {
      ...
      console.error(
        "Error fetching user details from users table:",
    ...
    const { data: latestAttendanceRecord, error: attendanceRecordError } =
      (await supabase
        .from("attendance")
        ...
        .limit(1)
        .single()) as {
        data: LastAttendanceRecord | null;
    ...
    if (attendanceRecordError && attendanceRecordError.code !== "PGRST116") {
      ...
      const { data: sessionDetails, error: sessionDetailsError } =
        ...
        `,
          ...
          .single()) as {
          data: LastAttendanceSessionDetails | null;
          error: PostgrestError | null;
        };

      if (sessionDetailsError && sessionDetailsError.code !== "PGRST116") {
        console.error(
          "Error fetching session details for last attendance:",
          sessionDetailsError.message,
        );
    ...
    if (enrolledCourseIds.length > 0) {
      ...
      if (coursesError) {
        console.error(
          "Error fetching enrolled courses details:",
          coursesError.message,
        );
        setErrorMessage((prev) =>
          prev
            ? prev + " Also failed to load full course details."
            : "Failed to load some course details.",
        );
      } else {
    ...
    const { data: courseIntakesForCourses, error: courseIntakesError } =
      await supabase
        .from("course_intakes")
        .select("id")
    ...
    if (courseIntakesError) {
      console.error(
        "Error fetching course intakes for enrolled courses:",
        courseIntakesError.message,
      );
      setLoading(false);
      setErrorMessage(
        "Failed to load course intakes relevant to your enrollment.",
      );
      return;
    }

    const relevantCourseIntakeIds =
      courseIntakesForCourses?.map((ci) => ci.id) || [];
    ...
    if (sessionsError) {
      console.error(
        "Error fetching all relevant sessions:",
    ...
    const sessions: SessionWithCourseAndIntake[] = (fetchedSessions ||
      []) as SessionWithCourseAndIntake[];
    ...
    const categorizedSessions = sessions.map((session) => {
      const sessionStart = session.attendance_window_start
        ? new Date(session.attendance_window_start)
        : null;
      const sessionEnd = session.attendance_window_end
      ...
      if (
        session.is_attendance_active &&
        sessionStart &&
        sessionStart < currentTime &&
        (!sessionEnd || currentTime < sessionEnd)
      ) {
        status = "active";
      }
    ...
    setAllSessions(
      categorizedSessions as (SessionWithCourseAndIntake & {
        status: "active" | "upcoming" | "closed";
      })[],
    );

    // Auto-select an active session if there's exactly one
    const activeSessions = categorizedSessions.filter(
      (s) => s.status === "active",
    );
    if (activeSessions.length === 1) {
      const autoSelectedSession =
        activeSessions[0] as SessionWithCourseAndIntake;
      ...
      if (markedError && markedError.code !== "PGRST116") {
        console.error(
          "Error checking existing attendance status for auto-selected session:",
          markedError.message,
        );
  ...
  const handleSessionSelect = useCallback(
    async (session: SessionWithCourseAndIntake & { status: string }) => {
      // Prevent selection if session is not active
      if (session.status !== "active") {
        toast.error("Only active sessions can be selected for attendance.", {
          id: "select-inactive-session",
        });
        return;
      }
      ...
      if (markedError && markedError.code !== "PGRST116") {
        console.error(
          "Error checking existing attendance status for selected session:",
          markedError.message,
        );
        setErrorMessage("Error checking attendance status.");
        ...
        .on(
          ...
          (payload) => {
            ...
            if (payload.new.is_attendance_active === false) {
              ...
              setErrorMessage(
                "The attendance session has been closed by the lecturer.",
              );
              setAttendanceStatus(null);
  ...
  const handleQrScanSuccess = async (scannedText: string) => {
    if (!currentSession || !currentSession.id) {
      setErrorMessage(
        "No active session selected to mark attendance for. Please select a session.",
      );
      setAttendanceStatus("error");
    ...
    return (
      <div className="flex justify-center items-center min-h-screen bg-gray-50 text-gray-800 antialiased font-sans">
        <p
          ...
          aria-live="polite"
        >
          Loading sessions and your enrollments...
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50 antialiased font-sans flex flex-col items-center justify-center py-8 px-4">
        ...
        >
          ...
          <h2 className="text-3xl font-bold mt-4 text-green-700">
            Attendance Recorded!
          </h2>
          <p className="mt-2 text-gray-700">
            You are present for{" "}
            <span className="font-semibold">
              {currentSession?.course_intakes?.courses?.title ||
  ...
  if (errorMessage) {
    return (
      <div className="min-h-screen bg-gray-50 antialiased font-sans flex flex-col items-center justify-center py-8 px-4">
        <div
          className="text-center p-8 bg-red-50 rounded-xl shadow-lg max-w-md w-full border-l-4 border-red-500"
          role="alert"
          aria-live="assertive"
        >
      ...
      <div className="min-h-screen bg-gray-50 text-gray-800 antialiased font-sans flex flex-col items-center py-8 px-4">
        <div className="container mx-auto p-4 max-w-2xl text-center bg-white rounded-xl shadow-lg w-full">
          <h2 className="text-2xl font-semibold mb-4 text-center text-gray-900">
            Mark Attendance for{" "}
            {currentSession.course_intakes?.courses?.title || "Selected Course"}{" "}
            -{" "}
            {currentSession.course_intakes?.intakes?.label || "No Intake Label"}
          </h2>
      ...
      <div className="container mx-auto p-4 max-w-3xl md:max-w-4xl lg:max-w-5xl text-center bg-white rounded-xl shadow-lg w-full">
        <div className="p-8" role="status" aria-live="polite">
          ...
          {lastMarkedAttendance && (
            <div className="mt-8 p-4 bg-blue-100 rounded-lg text-left border-l-4 border-[#329EE8]">
              ...
              <p className="text-blue-700">
                You marked attendance for{" "}
                <span className="font-bold">
                  {lastMarkedAttendance.courseTitle}
                </span>
                <br /> on{" "}
                <span className="font-bold">
                  {lastMarkedAttendance.markedAt}
                </span>
                .
              </p>
            </div>
          ...
          {enrolledCourses && enrolledCourses.length > 0 ? (
            <div className="mt-6 text-left p-4 bg-gray-50 rounded-lg shadow-inner">
              ...
              <p className="mt-4 text-sm text-gray-500">
                You can also check your course schedule for upcoming session
                times.
              </p>
            </div>
          ) : (
          ...
          {allSessions && allSessions.length > 0 ? (
            <div className="mt-8 text-left">
              ...
              <ul className="space-y-4">
                {allSessions.map((session) => (
                  <li
                    key={session.id}
                    className="p-5 border border-gray-200 rounded-xl bg-white shadow-sm hover:shadow-md transition-shadow duration-200 ease-in-out flex flex-col sm:flex-row justify-between items-start sm:items-center"
                  >
                    <div className="flex-grow">
                      <p className="font-bold text-xl text-gray-900 leading-tight">
                        {session.course_intakes?.courses?.title ||
                          "Unknown Course"}{" "}
                        ({session.course_intakes?.courses?.code || "N/A"})
                      </p>
                      <p className="text-md text-gray-700 mt-1">
                        Intake:{" "}
                        {session.course_intakes?.intakes?.label || "N/A"}
                      </p>
            ...
            <p className="mt-8 text-gray-600 p-4 bg-gray-50 rounded-lg shadow-inner">
              No active, upcoming, or recently closed sessions found for your
              enrolled courses.
            </p>
          )}
