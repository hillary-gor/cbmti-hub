 ...
 */
export async function markAttendance(sessionId: string, scannedQrValue: string): Promise<MarkAttendanceResponse> {
  const supabase = await createClient(); // Initialize Supabase client for server actions
  const { data: { user }, error: userError } = await supabase.auth.getUser(); // Get authenticated user
  ...
  if (profileError || !profile || profile.role !== 'student') {
    console.error('Authorization failed: User not a student or profile not found.', profileError);
    return { success: false, error: 'Access denied. Only students can mark attendance.' };
  }

  // 3. Fetch Session Details for Validation
  const { data: session, error: sessionError } = await supabase
    .from('sessions')
    .select('last_qr_code_value, attendance_window_start, attendance_window_end, is_attendance_active')
    .eq('id', sessionId)
    .single();

  if (sessionError || !session) {
    console.error(`Session fetch failed for ID ${sessionId}:`, sessionError?.message);
    return { success: false, error: 'Session not found or details unavailable.' };
  }

  const currentTime = new Date();
  // Parse window start and end times, handling potential null values
  const windowStart = session.attendance_window_start ? new Date(session.attendance_window_start) : null;
  const windowEnd = session.attendance_window_end ? new Date(session.attendance_window_end) : null;
  ...
  if (
    ...
    console.warn(`Attendance window validation failed for session ${sessionId}. Is active: ${session.is_attendance_active}, Start: ${windowStart?.toISOString()}, End: ${windowEnd?.toISOString()}, Current: ${currentTime.toISOString()}`);
    return { success: false, error: 'Attendance is not currently open for this session. It might be closed, not yet started, or expired.' };
  }

  // 5. Validate QR Code Match
  if (!session.last_qr_code_value || session.last_qr_code_value !== scannedQrValue) {
    console.warn(`QR code mismatch for session ${sessionId}. Expected: ${session.last_qr_code_value}, Received: ${scannedQrValue}`);
    return { success: false, error: 'Invalid or expired QR code. Please scan the latest QR code.' };
  }
  ...
  if (existingAttendanceError && existingAttendanceError.code !== 'PGRST116') { // PGRST116 means 'No rows found'
    console.error('Error checking existing attendance:', existingAttendanceError);
    return { success: false, error: 'Database error while checking your previous attendance.' };
  }
  if (existingAttendance) {
    console.warn(`Duplicate attendance detected for student ${user.id} in session ${sessionId}.`);
  ...
  const ipAddress = (await headers()).get('x-forwarded-for') || (await headers()).get('x-real-ip') || (await headers()).get('cf-connecting-ip');
  ...
  if (insertError) {
    ...
    return { success: false, error: 'Failed to mark attendance due to a database issue.' };
  }

  return { success: true, message: 'Attendance marked successfully!', attendanceId: newAttendance.id };
}